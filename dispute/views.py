...
class DisputeDetailsViews(APIView):
    def get(self, request, id = None):
        ...
        if page in ['true', True, "True"]:
            ...
            invoice = Invoice.objects.filter(created_date__gte=two_months_ago).order_by('-created_date')
            ...
            if from_date or to_date or customer_id:
                dispute = Dispute.objects.filter(created_date__range=[from_date, to_date],
    ...
    def post(self, request, id =None):
        try:
            dispute_serializer = DisputeSerialzer(data=request.data)
