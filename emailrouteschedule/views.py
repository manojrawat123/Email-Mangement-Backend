...
class EmailSheduleViews(APIView):
    ...
    def get(self, request, pk=None):
        if pk is not None:
            data = EmailRouteSchedule.objects.get(ScheduleID= pk)
            serializer = EmailSheduleSerializers(data)
            return Response(serializer.data)
        else:
            data = EmailRouteSchedule.objects.all()
    ...
    def put(self, request, pk):
        try:
            email_schedule = EmailRouteSchedule.objects.get(pk=pk)
...
class SendScheduleEmail(APIView):
    def post(self, request, id =None):
        try:
            data = request.data
            email_list = list(Customer.objects.filter(id__in=data['schedule_customer']).values_list('rates_email', flat=True))
            top_route_name = Route.objects.filter(top_route_name = data.schedule_route_id)
            template = EmailTemplate.objects.get(TemplateID = data.get('schedule_template'))
